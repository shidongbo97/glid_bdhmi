...
if isfield(opt, 'numMaxIters')
    numMaxIters = opt.numMaxIters;
else
    numMaxIters = 20000;
end
...
%% make sure the mesh does not contain any isolated vertex, it will interfere with the CETM optimization
assert( all( sparse(1, f, 1, 1, nv) ), 'there are isolated vertices in the mesh, remove them first!');
...
if ~exist('Theta', 'var')  % default, free boundary condition
