...
const char* OptimizationMethodNames[] = {"Gradient Descent", "AQP", "Newton", "Newton SPDH", "Newton Eigen" };
...
template<class Complex, class real>
std::vector<double> cuHarmonicDeform(const Complex *d_invM, const Complex *d_D2, const Complex *d_C2, const Complex *d_bP2P, Complex *d_phipsyIters, const int *hessian_samples,
    int m, int mh, int n, const std::vector<int> &cageOffsets, int nP2P, int isoEnergyType, real isoepow, real lambda, real aqpKappa, int nIter, 
    ...
    int reportIterationStats = 1, int linearSolvePref = LINEAR_SOLVE_PREFER_CHOLESKY, real deltaFixSPDH = 1e-15)
{
    ...
    printf("Config: %s, %s, %s, %.1f%%HS, wtP2P: %.1e, PDfix=%.1e\n",
        OptimizationMethodNames[optimization_method], IsometricEnergyNames[isoEnergyType], linearSolvePrefStr[linearSolvePref], mh*100. / m, lambda, deltaFixSPDH);
    ...
    auto fMyEnergy = [d_fzgz0, d_fgP2P, lambda, d_bP2P, m, nP2P, isoEnergyType, isoepow, softP2P](const Complex *dfzgz, real *t, const Complex *dfgP2P, real *en) { 
        p2p_harmonic_full_energy(d_fzgz0, dfzgz, d_fgP2P, dfgP2P, d_bP2P, m, nP2P, t, en, isoEnergyType, isoepow, lambda, softP2P, 1);
    };
    ...
    for (int it = 0; it < nIter; it++)  {
        ...
        if (aqpAccel && flagCanAccel) { // AQP acceleration
            ...
            if (needValidateMap) {
                ...
                harmonic_map_validate_bounds <<<1, 1 >>> (d_fzgz0, d_dfzgz, m, fzzgzz.data(), fzzgzz.data() + m * 2,
                    delta_L_fzgz.data(), sampleSpacings, validationData.nextSampleInSameCage, step, distortion_bounds.data() + it * 6, norm2dpp, enEvalsPerKernel);
        ...
        if (softP2P || it > min(nIter / 3, 15) || fp2pEn() < 1e-3) {
            harmonic_line_search<<<1,enEvalsPerKernel>>>(d_fzgz0, d_dfzgz, d_fgP2P, d_dfgP2P, validationData.nextSampleInSameCage, d_bP2P, m, nP2P, step, 
                d_en, isoEnergyType, isoepow, dgdotdpp_and_norm2dpp, lambda, softP2P, enEvalsPerKernel);
