...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, mxArray const *prhs[])
{
    ...
    // params: optional, includes, isoetype, isoepow, aqpKappa, nIter, enEvalsPerKernel, optimizationMethod

    mxInitGPU();   // Initialize the MathWorks GPU API.

    if (nrhs < 6)  mexError("not enough input");
    if (nrhs > 7 && !mxIsStruct(prhs[7])) mexError("params must be a struct");
    ...
    if (nrhs > 6) {
        if (mxIsSparse(prhs[6]) || mxIsGPUArray(prhs[6]) || mxGetClassID(prhs[6]) != mxINT32_CLASS)
            mexError("input: cageoffset must be cpu array of int32 type");

        std::copy_n((int*)mxGetData(prhs[6]), mxGetNumberOfElements(prhs[6]), std::back_inserter(cageOffsets));
    }

    // all parameters
    const mxArray *params = nrhs>7?prhs[7]:nullptr;
    ...
    if (runPrecision == mxDOUBLE_CLASS){
        ...
        //////////////////////////////////////////////////////////////////////////
        allStats = cuHarmonicDeform<Complex, real>((Complex const *)(mxGPUGetDataReadOnly(invM)), 
        ...
        allStats = cuHarmonicDeform<Complex, real>((Complex const *)(mxGPUGetDataReadOnly(invM)), 
            ...
            (Complex *)(mxGPUGetData(phipsyIters)),
            hessian_samples_rawp,
            m, mh, n, cageOffsets, nP2P, isoetype, isoepow, lambda, (real)aqpKappa, nIter, 
            validation_data,
            (const real*)sample_spacings_rawp, 
            enEvalsPerKernel, optimizationMethod,
            reportIterStats, linearSolvePref, deltaFixSPDH);
