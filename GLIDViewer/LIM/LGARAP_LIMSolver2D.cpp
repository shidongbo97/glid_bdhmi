...
void LGARAP_LIMSolver2D::computeLocalStep()
{
  ...
  Eigen::Matrix<double, 3, 2> uu;
  Eigen::Matrix2d rot;
  Eigen::Matrix3d cc;
  cc.fill(0);
  Eigen::MatrixX2d CovMat;
  ...
  for (int t=0;t<numTriangles;t++)
  {
	...
	Eigen::Vector2d rotv(CovMat(0, 0) + CovMat(1, 1), CovMat(0, 1) - CovMat(1, 0));
	rotv.normalize();

	//Eigen::Matrix2d rot2;
	//rot2 << rotv(0), -rotv(1), rotv(1), rotv(0);
	//double err = (rot - rot2).squaredNorm();
...
void LGARAP_LIMSolver2D::prepareProblemData(std::vector<int>& hessRowIdx, std::vector<int>& hessColIdx)
{
  ...
  igl::cotangent(*mesh->InitalVertices,*mesh->Triangles,CotanWeights);
