...
public:
    ...
    SimpleSparseMatrix();
    SimpleSparseMatrix(int r, int c, int nz, const int* rowstart, const int* colidxs, const T *vals);
    ...
    int nonZeros() { return numNonzero; }
    const T * valuePtr() { return values; }
    int * getAp() {
    ...
    void display() {
        for (int i = 0; i < nr; i++)
            for (int k = rowStart[i]; k < rowEnd(i); k++)
                printf("(%d,%d) %g", i, column[k], values[k]);
    }
    ...
    void zeroOutColumns(const std::vector<bool> &cols) {
